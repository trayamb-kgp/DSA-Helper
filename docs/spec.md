...
2. ~~**Distribution**~~ — settled 2026-09-03: **Chrome Web Store**, source public on GitHub under **PolyForm Strict 1.0.0** (source-available; changed from MIT on 2026-09-06, [D048](decisions.md#d048)). The listing copy, the per-permission justifications and the submission checklist are in [STORE-LISTING.md](STORE-LISTING.md); the first release goes out at a 10% staged rollout ([D030](decisions.md)). Two values are still to be substituted before submission — the repository slug and the published contact address, both one line in `src/core/links.ts` ([D046](decisions.md)).
